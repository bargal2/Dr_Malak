...
// 🎯 قم بتعديل تاريخ بداية الحكاية هنا فقط بفرمتة (YYYY-MM-DD)
const START_DATE_STRING = '2026-05-26';

const LoveTimer = () => {
    // تحويل النص إلى كائن Date للحسابات
    const startDate = new Date(`${START_DATE_STRING}T00:00:00`);

    // تنسيق التاريخ للعرض النصي العلوي بصيغة (DD/MM/YYYY)
    ...
    return (
        <section className="relative min-h-screen w-full flex flex-col items-center justify-center py-20 px-6 bg-[#030014] overflow-hidden font-sans">
            ...
            {/* 1. التاريخ العلوي (Luxury Badge) يتغير تلقائياً حسب المتغير */}
            <motion.div
            ...
            >
                ...
                <div className="px-6 py-2 rounded-full border border-pink-500/20 bg-pink-500/5 backdrop-blur-md flex items-center gap-3 shadow-[0_0_20px_rgba(236,72,153,0.1)]">
                    ...
                    <span className="text-white font-mono text-xs md:text-sm tracking-[0.3em] font-bold">{formattedDate}</span>
                </div>
                <div className="h-[1px] w-8 bg-pink-500/50" />
            </motion.div>
