...
// ✍️ 1. الاسم المعروض في العنوان
const HERO_NAME = "Dr Malak";

// ✍️ 2. أسطر الرسالة (يمكنك تعديل أي سطر أو إضافة أسطر جديدة)
const HERO_LINES = ["الخوف الحقيقي والزعل هو من عدم المحاوله انما طالما حاولتي فا م من حقك توجعي في نفسك حقا عليا انا في كل لحظه زعل وخوف مريتي بيها وكل نجاح بدايته لازم تكون محرقه ..ربنا عوضني بيكي في اكتر وقت كنت اصلا كارهه فكره اني اعرف اي بنت ول احب وفاجاه قلبي بقي ملكك ومحدش عارف يدخله غيرك ويوم ميلادك لسه هنحتفل بي سوا التورطه زي ماحبيتي دكتوره والورد كل الوعود هوفهالك يانور عيني يديمك ليااا ياحلي واجمل دكتوره في الدنياا♥️♥️♥️♥️"];
...
const Hero = () => {
    ...
    const fullText = HERO_LINES.join("\n\n");

    const [displayedText, setDisplayedText] = useState("");

    useEffect(() => {
        let currentIndex = 0;
        setDisplayedText(""); // إعادة تصفير النص عند تغيير الرسالة
    ...
    return (
        ...
        >
            ...
            <motion.div style={{ y: y1, opacity }} className="z-20 flex flex-col items-center text-center px-4 max-w-xl mx-auto w-full">
                ...
                {/* الاسم (مرتبط بالمتغير HERO_NAME) */}
                <motion.h1
